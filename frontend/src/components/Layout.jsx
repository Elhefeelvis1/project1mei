...
import { LayoutDashboard, ShoppingCart, Package, ArrowRightLeft, Users, FileText, LogOut, Wrench, History, ChevronLeft, ChevronRight, Settings, UserCog, Locate } from 'lucide-react';
...
const Layout = () => {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 font-sans">
      {!isSales && !isPurchase && <Sidebar user={user} />}
      <div className="flex-1 flex flex-col min-h-screen overflow-hidden">
        {(isSales || isPurchase) && <Navbar user={user} />}
        <main className={`flex-1 overflow-x-hidden overflow-y-auto bg-gray-50 p-8 ${(!isSales && !isPurchase) && 'pl-70'} ${(isSales || isPurchase) && 'pt-18'}`}>
          <Outlet context={{ user }} />
        </main>
      </div>
    </div>
